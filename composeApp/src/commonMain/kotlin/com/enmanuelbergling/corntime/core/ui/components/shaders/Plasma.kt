package com.enmanuelbergling.corntime.core.ui.components.shaders
...
val KLKSimplePlasmaShader = """ //most artistic an colorful one
    ...
    float4 main(  in float2 fragCoord )
    {
    	...
    	float4 fragColor = float4(r,g,b,1.0);
        return fragColor;
    }
""".trimIndent()
...
val LavaPlasmaShader = """
    ...
    float4 main(  in float2 fragCoord )	{
        ...
        float4 fragColor = float4(pow(r,R),pow(g,R),pow(b,R),1.0);
        return fragColor;
    }
""".trimIndent()
...
val GoldenFlowShader = """
    ...
    float4 main( in float2 fragCoord )
    {
    	...
    	float4 fragColor=float4(f+0.6,0.2+0.75*f,0.2,1.0);
        return fragColor;
    }
""".trimIndent()